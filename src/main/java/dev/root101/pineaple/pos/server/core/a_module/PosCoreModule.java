 ...
 */
public class PosCoreModule extends DefaultAbstractModule {

    private static final Injector inj = Guice.createInjector(new PosCoreInjectorConfig());

    private static PosCoreModule INSTANCE;

    public static PosCoreModule getInstance() {
        if (INSTANCE == null) {
    ...
    public static PosCoreModule init(dev.root101.clean.core.app.modules.AbstractModule repoModule) {
        if (INSTANCE != null) {
            throw new AlreadyInitModule("Module ya inicializado");
        }
        INSTANCE = new PosCoreModule();
