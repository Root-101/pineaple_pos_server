 ...
 */
public class PosRepoExternalModule extends DefaultAbstractModule {

    private static Injector inj;

    private static PosRepoExternalModule INSTANCE;

    public static PosRepoExternalModule getInstance() {
        if (INSTANCE == null) {
    ...
    public static PosRepoExternalModule init(PosAreaJPARepo areaRepoExternal) {
        if (INSTANCE != null) {
            throw new AlreadyInitModule("Module ya inicializado");
        }
        inj = Guice.createInjector(
                new PosRepoExternalInjectorConfig(
                        areaRepoExternal
                )
        );
        INSTANCE = new PosRepoExternalModule();
        return getInstance();
    }
